...
class StackedHourglass_ACBE(nn.Module):
    def __init__(self, device, ms_stream_num, noisy_factor, expand, k, stack_num):
        super(StackedHourglass_ACBE, self).__init__()
        self.k = k
        ...
        self.ms = [MultiStream(device, self.ms_stream_num, [256, 64, 64], noisy_factor=self.noisy_factor, is_expand=self.expand, use_adaptor=True, adaptor_type='1') for sIdx in range(self.stack_num)]
...
def build_hourglass_acbe(stream_num, k, noisy_factor, expand, device, stack_num=3):
    return StackedHourglass_ACBE(device, stream_num, noisy_factor, expand, k, stack_num)
