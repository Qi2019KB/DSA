...
class CifarResNeXt_MS(nn.Module):
    """
    ResNext optimized for the Cifar dataset, as specified in
    https://arxiv.org/pdf/1611.05431.pdf
    """

    def __init__(self, device, cardinality, depth, num_classes, base_width, widen_factor=4, ms_stream_num=3, noisy_factor=0.2):
        """ Constructor
        Args:
        ...
        self.ms = MultiStream(device, self.ms_stream_num, [self.stages[3], 8, 8], noisy_factor=self.ms_noisy_factor)
        self.ms_fc = nn.ModuleList([
            nn.Sequential(nn.Linear(self.stages[3], self.nlabels)) for stIdx in range(self.ms_stream_num)])
    ...
    def forward(self, x):
        ...
        x = self.act(self.bn_1.forward(x))  # torch.Size([480, 64, 32, 32])
        x = self.stage_1.forward(x)  # torch.Size([480, 256, 32, 32])
        x = self.stage_2.forward(x)  # torch.Size([480, 512, 16, 16])
        x = self.stage_3.forward(x)  # torch.Size([480, 1024, 8, 8])
        ms_preds, ms_fs_p = [], []
        x = self.ms.pre(x)
        for stIdx in range(self.ms_stream_num):
            ms_f, ms_f_p = self.ms.forward(stIdx, x)
            ms_fs_p.append(ms_f_p)

            # predict
            ms_f = F.adaptive_avg_pool2d(ms_f, 1)
            ms_f = ms_f.view(-1, self.stages[3])
            ms_pred = self.ms_fc[stIdx](ms_f)
            ms_preds.append(ms_pred)
        return torch.stack(ms_preds, 0), torch.stack(ms_fs_p, 0)


def build_resnext_ms(cardinality, depth, width, num_classes, stream_num, noisy_factor, device):
    return CifarResNeXt_MS(device, cardinality, depth, num_classes, width, ms_stream_num=stream_num, noisy_factor=noisy_factor)
