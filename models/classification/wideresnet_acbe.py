...
class WideResNet_ACBE(nn.Module):
    def __init__(self, device, num_classes, depth=28, widen_factor=2, drop_rate=0.0, ms_stream_num=3, noisy_factor=0.2):
        ...
        self.ms = MultiStream(device, self.ms_stream_num, [channels[3], 8, 8], noisy_factor=self.ms_noisy_factor, use_adaptor=True, adaptor_type='0')
...
def build_wideresnet_acbe(depth, widen_factor, dropout, num_classes, stream_num, noisy_factor, device):
    return WideResNet_ACBE(device, num_classes, depth, widen_factor, dropout, ms_stream_num=stream_num, noisy_factor=noisy_factor)
