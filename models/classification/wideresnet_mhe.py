...
class WideResNet_MHE(nn.Module):
    def __init__(self, device, num_classes, depth=28, widen_factor=2, drop_rate=0.0, ms_stream_num=3):
        super(WideResNet_MHE, self).__init__()
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = self.block1(x)  # torch.Size([480, 32, 32, 32])
        x = self.block2(x)  # torch.Size([480, 64, 16, 16])
        x = self.block3(x)  # torch.Size([480, 128, 8, 8])
        ms_preds = []
...
def build_wideresnet_mhe(depth, widen_factor, dropout, num_classes, stream_num, device):
    return WideResNet_MHE(device, num_classes, depth, widen_factor, dropout, ms_stream_num=stream_num)
